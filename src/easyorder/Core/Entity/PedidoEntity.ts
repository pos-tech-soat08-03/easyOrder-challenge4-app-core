...
import {
  ...
  StatusPedidoEnum,
} from "./ValueObject/StatusPedidoValueObject";
...
export class PedidoEntity {
  ...
  constructor(
    ...
    if (!dataPedido || dataPedido instanceof Date === false) {
      throw new Error(
        "Data do pedido não informada ou inválida na montagem do pedido"
      );
    }
    ...
    if (!statusPagamento) {
      throw new Error(
        "Status do pagamento não informado na montagem do pedido"
      );
    }
  ...
  public setStatusPedido(status: StatusPedidoValueObject): void {
    ...
    if (status.getValue() === StatusPedidoEnum.RECEBIDO) {
      if (
        this.statusPedido.getValue() !== StatusPedidoEnum.AGUARDANDO_PAGAMENTO
      ) {
  ...
  public setStatusPagamento(novoStatus: StatusPagamentoEnum): void {
    ...
    if (novoStatus === StatusPagamentoEnum.PAGO) {
      if (this.statusPagamento !== StatusPagamentoEnum.PENDENTE) {
        throw new Error(
          "Somente é possível marcar como pago pedidos com status de pagamento pendente"
        );
      }
    }
