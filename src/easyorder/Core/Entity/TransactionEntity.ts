...
import {
  ...
  StatusTransacaoEnum,
} from "./ValueObject/StatusTransacaoValueObject";
...
export class TransactionEntity {
  ...
  constructor(
    ...
    try {
      ...
      if (dataCriacaoTransacao !== undefined)
      ...
      if (statusTransacao !== undefined) this.statusTransacao = statusTransacao;
        ...
        this.statusTransacao = new StatusTransacaoValueObject(
          StatusTransacaoEnum.PENDENTE
        );
  ...
  public setStatusTransacao(novoStatus: StatusTransacaoValueObject): void {
    ...
    if (novoStatus.getValue() === StatusTransacaoEnum.PENDENTE) {
      throw new Error(
        "Não é possível retornar o status de transacao para PENDENTE"
      );
    }
    if (novoStatus.getValue() === StatusTransacaoEnum.EM_PROCESSAMENTO) {
      if (this.statusTransacao.getValue() === StatusTransacaoEnum.PENDENTE) {
        ...
        throw new Error(
          "Não é possível retornar o status de transacao finalizada para EM PROCESSAMENTO"
        );
    }
    if (novoStatus.getValue() === StatusTransacaoEnum.PAGO) {
      if (
        this.statusTransacao.getValue() === StatusTransacaoEnum.EM_PROCESSAMENTO
      ) {
        this.statusTransacao = novoStatus;
      } else
