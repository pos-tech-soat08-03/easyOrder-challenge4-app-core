...
export class PedidoUsecases {
  ...
  public static async ListarPedidosPorStatus(
    ...
    const filter = {
      ...
      orderDirection:
        orderDirection as PedidoGatewayInterfaceFilterOrderDirection,
    };

    const filterStatusPedido = new StatusPedidoValueObject(
      statusPedido as StatusPedidoEnum
    );
  ...
  public static async CheckoutPedido(
    ...
    const transacaoAtualizada =
      await transactionGateway.atualizarTransactionsPorId(
        transacaoEnviada.getIdTransacao(),
        transacaoEnviada
      );
  ...
  public static async AdicionarComboAoPedido(
    ...
    if (bebidaId) {
      ...
      if (produtoBebida.getCategoria() !== CategoriaEnum.BEBIDA) {
        throw new ValidationErrorException(
          "Produto informado não é uma bebida"
        );
      }
    }
    ...
    if (sobremesaId) {
      ...
      if (produtoSobremesa.getCategoria() !== CategoriaEnum.SOBREMESA) {
        throw new ValidationErrorException(
          "Produto informado não é uma sobremesa"
        );
      }
    }
    ...
    if (acompanhamentoId) {
      produtoAcompanhamento = await produtoGateway.buscarProdutoPorId(
        acompanhamentoId
      );
      ...
      if (
        ...
        throw new ValidationErrorException(
          "Produto informado não é um acompanhamento"
        );
      }
    }
