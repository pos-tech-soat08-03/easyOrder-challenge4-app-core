...
import {
  ...
  StatusTransacaoEnum,
} from "../Entity/ValueObject/StatusTransacaoValueObject";
...
export class PagamentoUsecases {
  public static async ConfirmarPagamentoUsecase(
    transactionGateway: TransactionGatewayInterface,
    pedidoGateway: PedidoGatewayInterface,
    pagamentoService: PagamentoServiceInterface,
    payload: string
  ): Promise<{ transacao: TransactionEntity | undefined; mensagem: string }> {
    const transactionDTO: PagamentoDTO =
      await pagamentoService.handlePaymentResponse(payload);

    const transaction = await transactionGateway.buscarTransactionPorId(
      transactionDTO.id
    );
    if (transaction === undefined) {
      return { transacao: undefined, mensagem: "Transação não encontrada." };
    }
    const pedido = await pedidoGateway.buscaPedidoPorId(
      transaction.getIdPedido()
    );
    if (pedido === undefined || pedido === null) {
      return {
        transacao: undefined,
        mensagem: "Pedido associado à transação não foi encontrado.",
      };
    }
    if (
      transaction.getStatusTransacao() === StatusTransacaoEnum.PAGO ||
      transaction.getStatusTransacao() === StatusTransacaoEnum.NEGADO ||
      transaction.getStatusTransacao() === StatusTransacaoEnum.CANCELADO
    ) {
    ...
    if (transactionDTO.status === RetornoPagamentoEnum.APROVADO) {
      ...
      const transacaoSalva =
        await transactionGateway.atualizarTransactionsPorId(
          transactionDTO.id,
          transaction
        );
      if (!transacaoSalva) {
        return {
          transacao: undefined,
          mensagem: "Erro ao salvar a transação.",
        };
      }
      ...
      return {
        ...
        mensagem: "Transação confirmada e pedido atualizado",
      };
    }
    ...
    transaction.setStatusTransacao(
      new StatusTransacaoValueObject(StatusTransacaoEnum.NEGADO)
    );
    transaction.setMsgRetorno(payload);
    const transacaoSalva = await transactionGateway.atualizarTransactionsPorId(
      transactionDTO.id,
      transaction
    );
    if (!transacaoSalva) {
  ...
  public static async ListarTransacoesUsecase(
    transactionGateway: TransactionGatewayInterface,
    pedidoGateway: PedidoGatewayInterface,
    pedidoId: string
  ): Promise<{
    transacoes: TransactionEntity[] | undefined;
    mensagem: string;
  }> {
    const transactions = await transactionGateway.listarTransactionsPorPedido(
      pedidoId
    );
    if (transactions === undefined) {
      return {
        transacoes: undefined,
        mensagem: "Não foram encontradas transações para o pedido.",
      };
    }
    return {
      transacoes: transactions,
      mensagem: `Sucesso. ${transactions.length} Transações encontrada(s).`,
    };
  }
}
