...
export class PagamentosController {
  public static async ConfirmarPagamento(
    ...
    const { transacao, mensagem } =
      await PagamentoUsecases.ConfirmarPagamentoUsecase(
        transactionGateway,
        pedidoGateway,
        servicoPagamento,
        payload
  ...
  public static async ListarTransacoes(
    dbConnection: IDbConnection,
    idPedido: string
  ): Promise<string> {
    const transactionGateway = dbConnection.gateways.transactionGateway;
    const pedidoGateway = dbConnection.gateways.pedidoGateway;
    const { transacoes, mensagem } =
      await PagamentoUsecases.ListarTransacoesUsecase(
        transactionGateway,
        pedidoGateway,
        idPedido
      );
