...
export class PedidoAdapter {
  ...
  public static dataNotFound(mensagem: string): PedidoAdapter {
    return new PedidoAdapter(PedidoAdapterStatus.DATA_NOT_FOUND, {
      message: mensagem,
    });
  }

  public static systemError(mensagem: string): PedidoAdapter {
    return new PedidoAdapter(PedidoAdapterStatus.SYSTEM_ERROR, {
      message: mensagem,
    });
  }

  public static validateError(mensagem: string): PedidoAdapter {
    return new PedidoAdapter(PedidoAdapterStatus.VALIDATE_ERROR, {
      message: mensagem,
    });
  }

  public static successPedido(
    pedido: PedidoEntity,
    mensagem?: string
  ): PedidoAdapter {
    return new PedidoAdapter(PedidoAdapterStatus.SUCCESS, {
      mensagem: mensagem || "Pedido cadastrado com sucesso",
      pedido: PedidoAdapter.adaptJsonPedidoObject(pedido),
    });
  }

  public static successPedidos(
    ...
    return new PedidoAdapter(PedidoAdapterStatus.SUCCESS, {
      ...
      pedidos: pedidos.map((pedido) =>
        PedidoAdapter.adaptJsonPedidoObject(pedido)
      ),
    });
  }
